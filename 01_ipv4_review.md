...
- ARP を使って相手の MAC アドレスを識別して通信する
  - a. ARP テーブルに存在する IPv4 アドレスに送信する場合、ARP テーブルを見て相手の MAC アドレスを割り出す 
  - b. ARP テーブルに存在しない IPv4 アドレスに送信する場合、「このアドレスを持つホストは誰？」の問い合わせを同一ネットワーク内にブロードキャストする
    - b-2. ブロードキャストを受け取ったホストは、自分がそのアドレスを持っている場合にのみ応答する
    - b-3. 応答を受けたホストは、IPv4 アドレスと応答の MAC アドレス、残存期間を ARP テーブルに保存する

#### 別ネットワーク間 : ルートテーブルを使ってルータに中継を依頼する

- 1. 同一ネットワーク内のルータ宛てに通信する（中継を依頼）
  - 1-1-a. 宛先のネットワークがルートテーブルに存在する場合は、ルートテーブルにあるネクストホップの IPv4 アドレス（ルータ）宛てに通信する
  - 1-1-b. 宛先のネットワークがルートテーブルに存在しない場合は、デフォルトゲートウェイの IPv4 アドレス（ルータ）宛てに通信する
  - 1-2. ルータの IPv4 アドレスが ARP テーブルに存在しない場合は「同一ネットワーク内」で説明したとおりの方法で MAC アドレスを割り出す
- 2. ルータは次の宛先・経由先を見つけて転送する（中継を依頼）
  - 2-a. 自分自身が直接接続している IPv4 ネットワークが宛先の場合は、対象ホスト宛てに通信する
  - 2-b. 自分自身が直接接続していない IPv4 ネットワークが宛先の場合は、ルートテーブルを見て次のルータに転送する
    - 2-b-2. ルートテーブルにない IPv4 アドレス宛ての場合は、デフォルトゲートウェイの IPv4 アドレス（ルータ）宛てに転送する
- 3. 宛先から送信元への通信（応答）も同じ流れで通信する
