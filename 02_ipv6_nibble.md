...
- 未定義アドレス
  - ::/128
  - アドレスが割り当てられていない状態
- グローバルユニキャストアドレス（GUA） : インターネット通信用アドレス（パブリックアドレス）
  - 上記以外のアドレス範囲
...
- ユニークローカルユニキャストアドレス（ULA）: 組織内のみで通信するためのアドレス（ルータ越え可能）
  ...
  - ~~AWS の VPC では使えない~~
    - IPAM（IP Address Manager）を使って割り当てられるようになった
...
- 複数のネットワークやグローバル（パブリック）アドレスを重ねて 1 つの VPC サブネットに割り当てることはできない
  ...
  - ~ULA を使うことはできない~
    - 2024/8 のアップデートで、IPAM（IP Address Manager）を使って IP アドレスの管理を行うケースでは ULA が使用可能に
      - 同一ホストで GUA と併用できるように
- 通常のインターネットゲートウェイを使うと、NACL・セキュリティグループの設定次第では IPv6 アドレスを持つホスト宛てに外部からアクセスが可能だが、Egress-Only インターネットゲートウェイを使うことで外部からのアクセスを禁止することが可能
  - IPv4 通信における NAT ゲートウェイと同じような状態に
    ...
    - ただしアドレス変換は行われない
- IPv6 専用 VPC であっても VPC 自体には IPv4 アドレスの割り当てが必要
  - （VPC Lattice 以外で）VPC 間を接続して通信する場合は割り当てた IPv4 アドレス空間を重複させてはいけない
    - そのため、IPv4 プライベートアドレス空間の捻出が難しいケースではキャリアグレード NAT 用の 100.64.0.0/10 の範囲を割り当てるのが一般的
  - サブネットには IPv4 アドレスを割り当てなくても問題はない
  - AWS の内部的な制御のため、EC2 などを IPv6 VPC に接続すると IPv4 のリンクローカルアドレスも使用される
    - VPC に割り当てた IPv4 アドレス空間とは無関係
